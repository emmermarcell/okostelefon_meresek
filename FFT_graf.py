...
# adatok betoltese
data = np.loadtxt('meresek\\sawtooth' +str(f) +'.csv', delimiter='\t', skiprows=1)
raw_data = np.loadtxt('meresek\\sawtooth' +str(f) +'_raw_data.csv', delimiter='\t', skiprows=1)
...
theo_signal = np.amax(Raw_kiteres) * signal.sawtooth(2 * np.pi * f * t)
...
# grafikon mentese
plt.savefig('sawtooth' +str(f), dpi=300)