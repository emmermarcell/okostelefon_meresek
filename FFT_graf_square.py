...
# adatok betoltese
data = np.loadtxt('meresek\\square' +str(f) +'.csv', delimiter='\t', skiprows=1)
raw_data = np.loadtxt('meresek\\square' +str(f) +'_raw_data.csv', delimiter='\t', skiprows=1)
...
theo_signal = np.amax(Raw_kiteres) * signal.square(2 * np.pi * f * t)
...
# grafikon mentese
plt.savefig('square' +str(f), dpi=300)