...
# adatok betoltese
data = np.loadtxt('meresek\\triangle' +str(f) +'.csv', delimiter='\t', skiprows=1)
raw_data = np.loadtxt('meresek\\triangle' +str(f) +'_raw_data.csv', delimiter='\t', skiprows=1)
...
theo_signal = np.amax(Raw_kiteres) * signal.sawtooth(2 * np.pi * f * t, width = 0.5)
...
plt.title('Háromszszöghullám FFT, $\\nu = ' +str(f) +'$ Hz, Corr = ' + str(SigCor))
plt.xlabel('$\\nu$ (Hz)')
...
# grafikon mentese
plt.savefig('triangle' +str(f), dpi=300)